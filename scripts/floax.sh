...
if [ "$(tmux display-message -p '#{session_name}')" = "scratch" ]; then
    ...
    if tmux has-session -t scratch 2>/dev/null; then
        ...
        tmux popup -S fg=magenta -s fg=blue,bold -T "FloaX" -b rounded -E -w "$FLOAX_WIDTH" -h "$FLOAX_HEIGHT" "tmux attach-session -t scratch"
    else
        ...
        tmux popup -S fg=magenta -E -w "$FLOAX_WIDTH" -h "$FLOAX_HEIGHT" "tmux attach-session -t scratch"
